const { join } = require('path')
const ImageHandler = require('..')

const publicPath = '/myImageDirectory'
const output = join(__dirname, 'output', `${publicPath}`)

const handler = new ImageHandler({
  output,
  publicPath,
  sizes: [320, 640, 960],
  placeholderSize: 20,
})

describe('ResponsivImageHandler', () => {
  beforeEach(async function () {
    await handler.clear()
  })
  it('It handles a regular image', async () => {
    const image = await handler.optimizeImagePath('__tests__/files/image.jpg')
    expect(image).toMatchInlineSnapshot(`
      Object {
        "height": 2908,
        "name": "image",
        "placeholder": "data:image/jpeg;base64,",
        "src": "/myImageDirectory/image-b124abd136fffe44501bac559c7dd89a-320.jpg",
        "srcSet": "/myImageDirectory/image-b124abd136fffe44501bac559c7dd89a-320.jpg 320w, /myImageDirectory/image-b124abd136fffe44501bac559c7dd89a-640.jpg 640w, /myImageDirectory/image-b124abd136fffe44501bac559c7dd89a-960.jpg 960w",
        "webpSrcSet": "/myImageDirectory/image-b124abd136fffe44501bac559c7dd89a-320.webp 320w, /myImageDirectory/image-b124abd136fffe44501bac559c7dd89a-640.webp 640w, /myImageDirectory/image-b124abd136fffe44501bac559c7dd89a-960.webp 960w",
        "width": 3882,
      }
    `)
  })
  it('It handles a vector image', async () => {
    const image = await handler.optimizeImagePath('__tests__/files/vector.svg')
    expect(image).toMatchInlineSnapshot(`
      Object {
        "height": 128,
        "name": "vector",
        "placeholder": "data:image/png;base64,",
        "src": "/myImageDirectory/00127173e2474c0803c5904421372b4d.svg",
        "width": 184,
      }
    `)
  })
})
